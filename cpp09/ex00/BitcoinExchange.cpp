...
void BitcoinExchange::ownCoin(std::string file) {
  ...
  while (getline(config, read)) checkInfo(read);
}
...
bool BitcoinExchange::checkDate(const std::string& date, int& year, int& month,
                                int& day) {
...
void BitcoinExchange::printData(const std::string& date, double n) {
  double res = 0;
  std::map<std::string, double>::const_iterator iter = coinData.find(date);
...
BitcoinExchange::BitcoinExchange(char* file) { checkFile(file); }
